...
const (
	generatedBanner = "// Code generated by generator. DO NOT EDIT."
	headerFileName  = "interop_array.h"
	csharpFileName  = "interop_bindings.g.cs"
)
...
func (g Generator) CLibrary(outDir string) {
	...
	csharpPath := filepath.Join(outDir, csharpFileName)
	if err := os.WriteFile(csharpPath, []byte(state.renderCSharp()), 0o644); err != nil {
		panic(err)
	}
}
...
func cleanGeneratedFiles(outDir string) error {
	...
	for _, entry := range entries {
		...
		if name == headerFileName || name == csharpFileName {
...
func (s *generatorState) renderCSharp() string {
	var out strings.Builder
	...
	out.WriteString("internal static class Native\n")
	out.WriteString("{\n")
	out.WriteString("    private const string LibraryName = \"plugin\";\n\n")
	...
	for _, ti := range s.orderedTypes {
		...
		for _, mi := range ti.Methods {
			...
			out.WriteString("    [DllImport(LibraryName, CallingConvention = CallingConvention.Cdecl)]\n")
			out.WriteString(fmt.Sprintf("    internal static extern %s %s(%s);\n\n", mi.CSharpExternReturnType, mi.WrapperName, strings.Join(params, ", ")))
		}
	}
	out.WriteString("}\n\n")
	...
	for _, ti := range s.orderedTypes {
		...
		out.WriteString("    }\n\n")
		out.WriteString(fmt.Sprintf("    public static %s New() => new %s(Native.%s());\n\n", ti.ExportName, ti.ExportName, constructorName(ti)))
		out.WriteString("    public void Release() => Native.Go_ReleaseHandle(Handle);\n\n")

		memberNames := newCSharpMemberNameSet(ti.ExportName)
		for _, mi := range ti.Methods {
			methodName := nextCSharpMemberName(csharpIdentifier(mi.Method.Name), memberNames)
...
func (s *generatorState) renderCSharpCallBody(out *strings.Builder, ti *typeInfo, mi *methodInfo, indent string) {
	...
	callExpr := fmt.Sprintf("Native.%s(%s)", mi.WrapperName, strings.Join(callArgs, ", "))
	...
	case 1:
		...
		case csKindString:
			out.WriteString(fmt.Sprintf("%sreturn InteropHelpers.ConsumeCString(result);\n", indent))
		case csKindStructHandle:
			out.WriteString(fmt.Sprintf("%sreturn new %s(result);\n", indent, ret.CSharpManagedType))
		case csKindArray:
			out.WriteString(fmt.Sprintf("%sreturn new GoArray<%s>(result.Data, result.Length, result.ElementSize);\n", indent, ret.ArrayElemCSharpType))
		default:
